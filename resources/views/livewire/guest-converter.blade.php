<div wire:init="ensureValidStep">
    <x-card variant="elevated">
        <x-stepper
            :steps="['File', 'Format', 'Settings', 'Convert']"
            :active="match ($step) {
                'format' => 'Format',
                'settings' => 'Settings',
                'convert', 'converting', 'completed', 'failed' => 'Convert',
                default => 'File',
            }"
            :handlers="array_filter([
                'File'     => in_array($step, ['format', 'settings', 'convert']) ? 'backToUploadSummary' : null,
                'Format'   => in_array($step, ['settings', 'convert']) ? 'backToFormatStep' : null,
                'Settings' => $step === 'convert' ? 'goToSettingsStep' : null,
            ])"
            class="mb-6"
        />
                    ...
                    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                        @foreach ($this->targetFormatCards as $card)
                            <button
                                ...
                                class="flex items-center gap-2.5 rounded-[var(--ca-radius-md)] border border-[var(--ca-border)] bg-white p-3 text-left transition hover:border-[var(--ca-primary)] hover:bg-[var(--ca-primary)]/5 ca-focus-ring cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed">
                                <x-file-icon :format="$card->targetFormat" size="sm" />
                                <span class="flex min-w-0 flex-col gap-0.5">
                                    ...
                                    <span class="text-xs leading-snug text-[var(--ca-muted)]">{{ $card->description }}</span>
                                </span>
                            </button>
                        @endforeach
                    </div>
