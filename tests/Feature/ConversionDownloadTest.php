...
it('returns 404 when completed job has no result file', function () {
    $user = User::factory()->create();
...
it('returns 404 when result file is missing from storage', function () {
    Storage::fake('local');

    $user = User::factory()->create();

    $resultFile = FileRecord::factory()->for($user)->create([
        'stored_path' => 'conversions/missing.jpg',
